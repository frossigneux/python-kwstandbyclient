...
"""
Command-line interface to the Kwstandby APIs
"""
...
from kwstandbyclient import client as kwstandby_client
from kwstandbyclient import exception
from kwstandbyclient.openstack.common import strutils
from kwstandbyclient import utils
from kwstandbyclient.v1.shell_commands import nodes
from kwstandbyclient import version as base_version

COMMANDS_V1 = {
    'node-list': nodes.ListNodes,
    'node-show': nodes.ShowNode,
    'node-update': nodes.UpdateNode,
}
...
class KwstandbyShell(app.App):
    """Manager class for the Kwstandby CLI."""
    CONSOLE_MESSAGE_FORMAT = '%(message)s'
    DEBUG_MESSAGE_FORMAT = '%(levelname)s: %(name)s %(message)s'
    log = logging.getLogger(__name__)

    def __init__(self):
        super(KwstandbyShell, self).__init__(
            description=__doc__.strip(),
            version=VERSION,
            command_manager=commandmanager.CommandManager('kwstandby.cli'), )
    ...
    def build_option_parser(self, description, version, argparse_kwargs=None):
        ...
        parser.add_argument(
            '--insecure',
            action='store_true',
            default=env('KWSTANDBYCLIENT_INSECURE', default=False),
            help="Explicitly allow kwstandbyclient to perform \"insecure\" "
    ...
    def initialize_app(self, argv):
        ...
        * validate authentication info
        ...
        super(KwstandbyShell, self).initialize_app(argv)
...
def main(argv=sys.argv[1:]):
    try:
        return KwstandbyShell().run(map(strutils.safe_decode, argv))
    except exception.KwstandbyClientException:
        return 1
