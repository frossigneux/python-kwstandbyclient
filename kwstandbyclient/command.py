...
class KwstandbyCommand(OpenStackCommand):

    """Base Kwstandby CLI command."""
    api = 'standby'
    log = logging.getLogger(__name__ + '.KwstandbyCommand')
    values_specs = []
    json_indent = None
    resource = None
    allow_names = True
    ...
    def __init__(self, app, app_args):
        super(KwstandbyCommand, self).__init__(app, app_args)
    ...
    def get_parser(self, prog_name):
        parser = super(KwstandbyCommand, self).get_parser(prog_name)
        return parser
...
class CreateCommand(KwstandbyCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        kwstandby_client = self.get_client()
        body = self.args2body(parsed_args)
        resource_manager = getattr(kwstandby_client, self.resource)
...
class UpdateCommand(KwstandbyCommand):
    ...
    def run(self, parsed_args):
        ...
        body = self.args2body(parsed_args)
        if self.allow_names:
            res_id = utils.find_resource_id_by_name_or_id(kwstandby_client,
                                                          self.resource,
                                                          parsed_args.id)
        else:
...
class DeleteCommand(KwstandbyCommand):
...
class ListCommand(KwstandbyCommand, lister.Lister):
    ...
    def args2body(self, parsed_args):
        ...
        if parsed_args.sort_by:
            if parsed_args.sort_by in self.list_columns:
                ...
                raise exception.KwstandbyClientException(msg)
        return params
    ...
    def retrieve_list(self, parsed_args):
        """Retrieve a list of resources from Kwstandby server"""
        kwstandby_client = self.get_client()
        body = self.args2body(parsed_args)
        resource_manager = getattr(kwstandby_client, self.resource)
...
class ShowCommand(KwstandbyCommand, show.ShowOne):
